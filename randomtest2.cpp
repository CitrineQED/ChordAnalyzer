...
void solve(int n){
    ...
    for(int i = 1; i < (N/2 -1); ++i){
        q_out[i] = d_out[i] * d_out[i+1]; //q_out[i]が負になっているとき、 out[i][0] は山の頂点
    }
    map<double,int> fftPeaks;
    for(int i = 1; i < (N/2 -1); ++i){
        if(q_out[i] <= 0){
            fftPeaks[out[i][0]] = i;
        }g:\kk.txt
...
void result(bool is_ok,int n){ // 結果を出力
    ofstream waOut("wa2.txt",ios_base::app);
    ofstream out("out2.txt",ios_base::trunc);
    if(is_ok){
        ++ac;
        cout << "I" << flush;
    }else{
